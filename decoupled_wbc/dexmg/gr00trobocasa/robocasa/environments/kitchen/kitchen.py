...
class Kitchen(ManipulationEnv, metaclass=KitchenEnvMeta):
    ...
    def __init__(
        ...
        # _ep_meta must exist before _load_model() is called (which happens inside
        # super().__init__() in this version of robosuite). In newer robosuite it is
        # initialised by the framework; here we seed it as an empty dict so that
        # _setup_model() can safely call self._ep_meta.get(...) without KeyError.
        self._ep_meta = {}
    ...
    def _reset_internal(self):
        ...
        # else:
        #     # Use anchor position without randomization (supports all robot types)
    ...
    def _setup_observables(self):
        ...
        """
        Override to debug camera sensor issues.
        """
        try:
            # Try to run the standard setup
            observables = super()._setup_observables()
            return observables
        except ValueError as e:
            print(f"\n{'='*60}")
            print(f"DEBUG: Observable Sensor Validation Failed")
            print(f"{'='*60}")
            print(f"Full Error: {e}")
            
            # Extract observable name from error string
            import re
            cam_name = None
            match = re.search(r"observable '([^']+)'", str(e))
            if match:
                obs_name = match.group(1)
                print(f"\nFailing Observable Name: '{obs_name}'")

                # Try to extract sensor ID / camera name from observable name
                # Usually: robot0_<camname>_image -> camera name is robot0_<camname>
                if obs_name.endswith('_image'):
                    cam_name = obs_name[:-6]  # remove '_image'
                    print(f"Inferred Camera Name: '{cam_name}'")

            if hasattr(self, 'sim') and self.sim is not None:
                all_cams = list(self.sim.model.camera_names)
                print(f"\nAll Cameras Present in Sim Model ({len(all_cams)} total):")
                for c in sorted(all_cams):
                    print(f"  - {c}")
                
                if cam_name and cam_name not in all_cams:
                    print(f"\n⚠️  MISSING CAMERA: '{cam_name}' is NOT in sim.model.camera_names!")
                elif cam_name:
                    print(f"\n✅ Camera '{cam_name}' IS present — strange why sensor is invalid...")
            
            # Also print what the robot thinks it should have
            for robot in self.robots:
                if hasattr(robot.robot_model, 'camera_configs'):
                    print(f"\nRobot '{robot.name}' defines these camera configs:")
                    for cn in sorted(robot.robot_model.camera_configs.keys()):
                        cfg = robot.robot_model.camera_configs[cn]
                        pb = cfg.get('parent_body', 'None')
                        print(f"  - {cn} | parent_body={pb}")
